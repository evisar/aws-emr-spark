...
if aws s3api head-bucket --bucket ccp-${component_id} 2>/dev/null; 
then
  aws s3api create-bucket --bucket ccp-${component_id} --region eu-west-2 --create-bucket-configuration LocationConstraint=eu-west-2
fi
